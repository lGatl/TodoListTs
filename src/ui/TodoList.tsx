...
const TodoList: React.FC = () => {
  ...
  const changeEstTerminee = (id: string, value: boolean) => {
    ...
    const storeObject = store.reduce(
      (total: { [id: string]: TodoType }, todo) => {
    ...
    setStore(
      Object.values({
        ...storeObject,
        [id]: { ...storeObject[id], estTerminee: value },
      }),
    );
  };
  const addToStore = () => {
  ...
  return (
    <form>
      ...
      <button onClick={addToStore} type="button">
        Add
      </button>
    </form>
  );
